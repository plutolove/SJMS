...
<%
    ...
    List<Note> thisMonthList = Note.DAO.getThisMonthNotes();    //本月的列表
    List<Note> olderList = Note.DAO.getOlderNotes();    //更久以前的列表
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <ul class="timeline">
                        <!-- timeline time label -->
                        <li class="time-label">
                            <span class="bg-blue">本月列表</span>
                        </li>
                        <!-- /.timeline-label -->
                        <!-- timeline item -->
                        <%
                            for(Note note : thisMonthList) {
                                //前面的小图标的样式
                                String iconClass = "fa fa-bell bg-blue";
                                String content = note.get(Note.content);
                                if (Note.LOGIN_ENABLE.equals(content)) {
                                    iconClass = "fa fa-user bg-green";
                                } else if (Note.LOGIN_DISABLE.equals(content)) {
                                    iconClass = "fa fa-user-times bg-red";
                                } else if (Note.JOB_ENABLE.equals(content)) {
                                    iconClass = "fa fa-unlock bg-green";
                                } else if (Note.JOB_DISABLE.equals(content)) {
                                    iconClass = "fa fa-lock bg-red";
                                }

                                boolean isSys = note.get(Note.author).equals(Note.system);
                        %>
                        <li>
                            <i class="<%=iconClass%>"></i>
                            <div class="timeline-item">
                                <span class="time"><i class="fa fa-clock-o"></i> <%=note.get(Note.time)%></span>
                                <% if(isSys) { %>
                                <h2 class="timeline-header"><font color="#3C8DBC"><%=note.get(Note.title)%></font></h2>
                                <% } else { %>
                                <h2 class="timeline-header"><a href="sys-note-view.jsp?id=<%=note.get(Note.id)%>"><%=note.get(Note.title)%></a></h2>
                                <% } %>
                                ...
                                <div class="timeline-footer">
                                    <% if(!isSys) { %>
                                    <div class="btn-group">
                                        <a class="btn btn-info" href="sys-note-edit.jsp?id=<%=note.get(Note.id)%>"><i class="fa fa-pencil"></i> 修改</a>
                                        <button type="button" class="btn btn-warning" onclick="sysNoteDelete(<%=note.get(Note.id)%>, $(this))"><i class="fa fa-times"></i> 删除</button>
                                    </div>
                                    <% } else { %>
                                    <button type="button" class="btn btn-warning" onclick="sysNoteDelete(<%=note.get(Note.id)%>, $(this))"><i class="fa fa-times"></i> 删除</button>
                                    <% } %>
                        ...
                        <!-- timeline time label -->
                        <li class="time-label">
                            <span class="bg-yellow">更久以前</span>
                        </li>
                        <!-- /.timeline-label -->
                        <!-- timeline item -->
                        <%
                            for(Note note : olderList) {
                                //前面的小图标的样式
                                String iconClass = "fa fa-bell bg-blue";
                                String content = note.get(Note.content);
                                if (Note.LOGIN_ENABLE.equals(content)) {
