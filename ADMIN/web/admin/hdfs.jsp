...
<%@ page import="org.ahu.edu.bigdatalab.scm.bean.HDFSDirectory" %>
<%@ page import="org.ahu.edu.bigdatalab.scm.model.User" %>
<%@ page import="org.ahu.edu.bigdatalab.scm.helper.HDFSHelper" %><%--
...
<%
    ...
    HDFSDirectory directory = (HDFSDirectory)request.getAttribute("directory");
    String path = request.getAttribute("path").toString();

    String pathes[] = path.split("/");      //路径的数组
...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            ...
            <button class="btn btn-primary" style="margin-bottom: 10px;" onclick="hdfsAdd('<%=path%>', $(this))"><i class="fa fa-folder-open"></i> 新建文件夹</button>
            <button class="btn btn-info" style="margin-bottom: 10px;" onclick="hdfsUpload('<%=HDFSHelper.URL%>', '<%=path%>')"><i class="fa fa-cloud-upload"></i> 上传文件</button>
                    ...
                    <div class="box">
                            ...
                            <h3 class="box-title">
                                ...
                                <%
                                    ...
                                    for(int i=0;i<pathes.length-1;i++){
                                        String item = pathes[i];
                                        if (item == null || item.equals("") || item.equals(" ")) {
                                            //可能出现空的字符串
                                            continue;
                                        }
                                        pathUrl = pathUrl + "/" +item;
                                 %>
                                    <li><a href="hdfs?path=<%=pathUrl%>"><%=item%></a></li>
                                <% } %>
                                    <li class="active"><%=pathes[pathes.length-1]%></li>
                                </ol>
                            </h3>
                        </div>
                        <!-- /.box-header -->
                        <div class="box-body">
                            <table id="hdfs_list" class="table table-bordered table-hover">
                                ...
                                <%
                                    for(HDFSDirectory.HDFSFile file : directory.getList()) {
                                        String iconClass = "fa fa-folder-open";
                                        if (file.getFtype() == HDFSDirectory.isFile) {
                                            iconClass = "fa fa-file";
                                        }
                                %>
                                <tr>
                                    <%
                                        if (file.getFtype() == HDFSDirectory.isFolder) {
                                            //判断是否是文件夹，不是的话不可以点击链接
                                    %>
                                    <td width="35%"><a href="hdfs?path=<%=(path + "/" + file.getFname()).replace("//", "/")%>"><i class="<%=iconClass%>"></i> <%=file.getFname()%></a></td>
                                    <%} else {%>
                                    <td width="35%"><a href="javascript:hdfsFileView('<%=(path + "/" + file.getFname()).replace("//", "/")%>')"><i class="<%=iconClass%>"></i> <%=file.getFname()%></a></td>
                                    <% } %>
                                    <td width="20%"><a href="javascript:userInfo(<%=userId%>)"><%=userName%></a></td>
                                    <td width="15%"><%=file.getMtime()%></td>
                                    <td width="15%"><%=file.getSize()%></td>
                                    <td width="30%">
                                        <div class="btn-group">
                                            <button type="button" class="btn btn-default" onclick="hdfsDelete('<%=path + "/" + file.getFname()%>', $(this))"><i class="fa  fa-close"></i> 删除</button>
                                            <button type="button" class="btn btn-default" onclick="hdfsRename('<%=path%>','<%=file.getFname()%>')"><i class="fa  fa-pencil"></i> 重命名</button>
                                            <%
                                                if (file.getFtype() == HDFSDirectory.isFile) {
                                            %>
                                            <a type="button" class="btn btn-default" href="hdfs/download?path=<%=path.replace("//", "/")%>&file=<%=file.getFname()%>" target="_blank"><i class="fa  fa-download"></i> 下载</a>
                                           <% } %>
                                        </div>
                                    </td>
                                </tr>
